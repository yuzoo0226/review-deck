...
これは動作確認用のサンプルです。この文をドラッグ選択すると「💬 コメント」「〜 取り消し線」「✍ 日本語を再考」が出ます。
コメントした箇所は黄色くハイライトされ、取り消し線を引いた箇所は赤い線で消され、日本語の再考を求めた箇所には青い波線が付き、右のマージンに一覧が並びます。
...
2. 「📄 資料を開く」でサイドピークに表示する。
   - **本文をドラッグ選択すると「💬 コメント」「〜 取り消し線」「✍ 日本語を再考」が出る**。コメントした箇所は
     黄色くハイライトされ、右のマージンに一覧が並ぶ（編集・対応済み・削除・すべて対応済み）。
   - **「〜 取り消し線」は削除提案**。押した時点で保存され、本文は赤い取り消し線、マージンには「✂ 削除提案」として並ぶ。
   - **「✍ 日本語を再考」は「この文は日本語として崩れているので書き直して」という指摘**（論文ドラフトのチェック用）。
     取り消し線と同じく押した時点で保存され、本文は青い波線、マージンには「✍ 日本語再考」として並ぶ。
   - 取り消し線・日本語再考は保存と同時に**理由／補足の入力枠が開く**。書いて `Enter` で確定（改行は `Shift+Enter`）。
     何も書かずに `Enter`・`Esc`・次の文の選択に進めば「理由なし」のまま残る（コメントと違って指摘は消えない）。
     後からマージンの「編集」で書き足すこともできる（空で保存すると理由が消える）。
   - ポップアップが出ている間は **キーでも選べる**: `C` = コメント / `D` = 取り消し線 / `J` = 日本語を再考 / `Esc` = 閉じる
     （Ctrl+C などの修飾キー付きは横取りしない）。
   ...
   - **画像・PDF を埋め込める**。`<REVIEW_DIR>` 配下のファイルを相対パスで参照する（例: `assets/<報告ID>/fig.png`）。
     md は `![キャプション](assets/…/fig.png)`、PDF は `![資料名](assets/…/doc.pdf)` を1行で書くとビューアごと埋め込まれる
     （文中に書くとリンクになる）。html 版は `<img src="assets/…">` がそのまま解決される。html 版の PDF 埋め込みは
     sandbox の中では表示できないので、「新しいタブで開く」カードに置き換わる。
     配信は `/reports/<相対パス>` から、png / jpg / gif / webp / svg / pdf / mp4 だけ（`<REVIEW_DIR>` の外やドットで始まるパスは返さない）。
...
3. **「レビューを Claude に返す」**: 未対応コメントを `/review-comments <報告ID>` 形式
   （`@ 位置` / `| 引用` / `→ コメント`。取り消し線は `→ ✂ [理由]`、日本語の再考は `→ ✍ [補足]` と記号だけで送り、
   記号の意味はメッセージ冒頭の凡例に1回だけ書く）にまとめ、
   送り先セッションを選んで tmux ペインへ注入する。
   - 記号の意味は凡例に短く書き、守ってほしい細かいルール（日本語再考なら「意味・数値・専門用語・引用番号は変えない」など）は
     review-comments skill が持つ。
...
[
  {
    ...
    "comments": [
      ...
      { "id": "c…", "kind": "rephrase", "quote": "日本語として崩れている本文", "text": "", "resolved": false,
        "occurrence": 1, "occurrenceTotal": 1, "loc": { "file": "…", "line": 31 } }
    ],
...
`kind` は `comment`（文章のコメント。`quote` は 120 字まで）、`strike`（取り消し線 = 削除提案。`text` は理由で空でもよく、
`quote` は削除範囲そのものなので 1000 字まで）、`rephrase`（日本語を再考 = 引用部分を意味を変えずに書き直させる。
`text` は補足で空でもよく、`quote` は書き直す範囲そのものなので 1000 字まで）のいずれか。
`kind` が無い古いコメントや未知の `kind` は `comment` として扱う。
...
- `POST /api/report/comment {id, quote, text, kind?, ...}` — コメント追加（`kind: "strike"` で取り消し線、`kind: "rephrase"` で日本語を再考。`action: edit|resolve|delete|resolve-all|locate` で操作）
