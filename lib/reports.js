...
//          comments: [{ id, kind: "comment"|"strike", quote, text, resolved, ... }],
...
//   古いデータは kind を持たないので comment 扱い。
...
export async function addComment(reportsDir, id, { anchorIndex, quote, text, kind, occurrence, occurrenceTotal, heading, loc }) {
  ...
  const comment = {
    ...
    kind: strike ? "strike" : "comment",
    anchorIndex: Number(anchorIndex) || 0,
    // 取り消し線は引用そのものが「どこまで消すか」なので、コメントより長く残す
    quote: String(quote || "").slice(0, strike ? 1000 : 120),
  ...
  if (strike ? comment.quote.trim().length < 2 : !comment.text) return null;
...
export async function mutateComment(reportsDir, id, commentId, action, text) {
  ...
  if (action === "delete") r.comments.splice(i, 1);
    ...
    // 取り消し線の text は「理由」なので、空で保存して理由を消せる
    if (!t && r.comments[i].kind !== "strike") return null;
    r.comments[i].text = t;
